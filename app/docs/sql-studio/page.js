    ...
    >
      ...
      <DocsSection id="workspace-controls" title="Workspace Controls And Support" intro="The top workspace buttons are shared across SQL Studio and Procedure Runner. They control documentation, support, tools, panels, and the local desktop server.">
        ...
        <DocsMiniSection title="Workbench Tools">
          ...
          <p><strong>Saved queries</strong> is your query library, stored by the app server so it survives a cleared browser. <strong>Save SQL</strong> (or <span className="docs-kbd">Ctrl</span> + <span className="docs-kbd">S</span> in the editor) asks for a name — type <code>Folder / Name</code> to file it in a folder, and save the same name again to replace it. Search filters by name, folder or SQL; <strong>This profile only</strong> hides queries saved for other profiles. <strong>Open</strong> loads a query into the editor, in a new tab when the current one already holds SQL. Older local scratchpads are imported into a <em>Scratchpads</em> folder the first time.</p>
      ...
      <DocsSection id="advanced" title="Advanced Operations And Object Analysis" intro="Advanced Operations create cross-object SQL templates. Object Analysis runs read-only metadata or profiling requests against the current connection.">
        ...
        <DocsMiniSection title="Read-only object analysis">
          <div className="docs-table">
            ...
            <div><strong>Schema compare</strong><span>Opens a dialog to compare the active object's columns and supported constraint/index metadata with an object on this connection or on any saved profile — for example the same table on dev and prod — and optionally their row counts. A profile that uses SQL login or Windows authentication asks for its password there; it is not stored. Use it before copying data or creating migration SQL.</span></div>
...
WHERE <review scope before execution>;`}</pre>
      ...
      <DocsSection id="results" title="Results Panel" intro="Results are designed for inspection first: read the shape, filter locally, inspect long values, then copy or export what you need.">
        ...
        <div className="docs-callout">
          ...
          Turning it on lets you edit cells in place — including key columns, which stay editable and just keep a small "key" badge as a hint, since a rename is matched by the key's original value and can never lose the row — mark rows for deletion with a red per-row button, and add new rows with <strong>+ New row</strong>. A pending-changes bar tracks the modified/deleted/new breakdown and enables <strong>Save changes</strong> / <strong>Discard edits</strong> only when there is something to act on.
        ...
        <div className="docs-callout docs-callout-info">
          <strong>Row matching without a primary key.</strong> When a table has no primary key or unique constraint — common on Fabric Warehouse, which doesn't enforce these constraints — the app matches a row by every visible, comparable column's original value instead. A note above the grid says when this fallback is active. Right before saving, each changed row is re-checked against the table to confirm its original values still match exactly one row; if they match zero (changed elsewhere since you loaded the grid) or more than one (the table has rows the app can't tell apart), the whole save is refused with an explanation and your staged edits are kept.
        </div>
      </DocsSection>
