...
import { deleteSavedConnection, initializeSavedConnectionsStore, listSavedConnections, resolveConnectionEnvironment, upsertSavedConnection } from './saved-connections-store.js';
import { deleteSavedQuery, listSavedQueries, upsertSavedQuery } from './saved-queries-store.js';
...
import { assertAcknowledgement, resolveProcedureAcknowledgement, resolveWriteAcknowledgement } from './write-acknowledgement.js';
...
function buildWriteReviewMessage(classification = {}, rowsAffected = null, { expectedText = '', environment = '' } = {}) {
  ...
  const prodNote = environment === 'prod' ? ' This connection is tagged PRODUCTION.' : '';
  const warningSuffix = `${prodNote}${warnings.length ? ` ${warnings.join(' ')}` : ''}`;
  ...
  if (action === 'BATCH') {
    ...
    const operations = Array.isArray(classification.actions) && classification.actions.length
      ...
      : '';
    return `This batch contains ${count || 'multiple'} SQL statements.${operations}${prodNote} Review every statement, type ${expectedText || 'RUN BATCH'}, then click Continue to execute.`;
  }
  ...
  if (classification.requiresAcknowledgement) {
    const rowSuffix = Number.isFinite(Number(rowsAffected)) && Number(rowsAffected) > HEIGHTENED_CONFIRM_LIMIT
      ...
      : '';
    return `${action} requires explicit acknowledgement.${rowSuffix}${warningSuffix} Review the SQL, type ${expectedText || `EXECUTE ${action}`}, then click Continue to execute.`;
  }
...
async function createProcedureConfirmationRecord({ sessionId, connection, procedure, parameters, expectedText = '', environment = '' }) {
  const normalizedConnection = normalizeConnectionInput(connection);
...
export async function postSchemaCompare(req, res) {
  ...
  const includeRowCounts = req.body?.includeRowCounts === true;
  // A row count that cannot be read (permissions, Lakehouse without partition stats) should not
  // throw away a schema comparison that worked, so each side degrades to null with a warning.
  const countRows = async (pool, objectName, side) => {
    try {
      const counted = await loadObjectRowCount(pool, objectName, { allowCountFallback: true });
      return { rowCount: counted.rowCount, exact: counted.exact, warning: '' };
    } catch (error) {
      return { rowCount: null, exact: false, warning: `${side} row count unavailable: ${error.message}` };
    }
  };
  ...
  try {
    const data = await withConnection(leftConnection, async (leftPool) => (
      withConnection(rightConnection, async (rightPool) => {
        const compared = await loadSchemaCompare(leftPool, rightPool, {
          leftObject,
          rightObject,
          objectType,
        ...
        const [left, right] = await Promise.all([
          countRows(leftPool, leftObject, 'Left'),
          countRows(rightPool, rightObject, 'Right')
        ]);
        const rowCounts = { left: left.rowCount, right: right.rowCount, leftExact: left.exact, rightExact: right.exact };
        const differs = left.rowCount !== null && right.rowCount !== null && left.rowCount !== right.rowCount;
        return {
          ...compared,
          rowCounts,
          rows: [
            { item: '(row count)', difference: differs ? 'row_count' : left.rowCount === null || right.rowCount === null ? 'unavailable' : 'same', left: left.rowCount, right: right.rowCount },
            ...(compared.rows || [])
          ],
          summary: { ...(compared.summary || {}), leftRows: left.rowCount, rightRows: right.rowCount },
          warnings: [...(compared.warnings || []), left.warning, right.warning].filter(Boolean)
        };
      })
    ));
    const sameTarget = buildConnectionFingerprint(leftConnection) === buildConnectionFingerprint(rightConnection);
    addAuditEntry({
      ...
      detail: sameTarget
        ? `Compared ${leftObject} to ${rightObject}.`
        : `Compared ${leftObject} to ${rightObject} on ${rightConnection.server}/${rightConnection.database}.`
    });
...
export async function postProcedures(req, res) {
  ...
  try {
    ...
    if (confirmToken) {
      ...
      const currentTarget = await resolveConnectionEnvironment(connection);
      if (currentTarget.environment === 'prod' && record.payload.environment !== 'prod') {
        await deleteConfirmation(confirmToken);
        return res.status(400).json({ success: false, error: 'This connection was marked as production after the procedure was prepared. Prepare it again.' });
      }
      try {
        assertAcknowledgement(record.payload.expectedText, req.body?.acknowledgement);
      } catch (error) {
    ...
    const procedureTarget = await resolveConnectionEnvironment(connection);
    const procedureExpectedText = resolveProcedureAcknowledgement(procedureTarget);
    const record = await createProcedureConfirmationRecord({
      sessionId: req.sessionId,
      connection,
      procedure: procedure.fullName,
      parameters: rawParameters,
      expectedText: procedureExpectedText,
      environment: procedureTarget.environment
    });
    addAuditEntry({
    ...
    return res.json({
      ...
      expectedText: procedureExpectedText,
      environment: procedureTarget.environment,
      profileName: procedureTarget.profileName,
      message: procedureExpectedText
        ? `Stored procedure ${procedure.fullName} is ready on a PRODUCTION connection. Review the parameters, type ${procedureExpectedText}, then click Run procedure.`
        : `Stored procedure ${procedure.fullName} is ready. Review the parameters and click Run procedure to continue.`
    });
  } catch (error) {
    addAuditEntry({
      event: 'procedure_execute',
...
export async function postQuery(req, res) {
  ...
  try {
    ...
    if (confirmToken) {
      ...
      // The profile may have been tagged prod after this preview was made; its phrase would
      // then be too weak, so a fresh preview (with the prod phrase) is required.
      const currentTarget = await resolveConnectionEnvironment(connection);
      if (currentTarget.environment === 'prod' && record.payload.environment !== 'prod') {
        await deleteConfirmation(confirmToken);
        return res.status(400).json({ success: false, error: 'This connection was marked as production after the preview. Preview the write again.' });
      }
      try {
        assertAcknowledgement(record.payload.expectedText, req.body?.acknowledgement);
      } catch (error) {
    ...
    const target = await resolveConnectionEnvironment(connection);
    const { expectedText } = resolveWriteAcknowledgement({ classification, environment: target.environment, profileName: target.profileName });
    ...
    if (classification.directConfirmOnly) {
      ...
      return res.json({
        ...
        message: buildWriteReviewMessage(classification, null, { expectedText, environment: target.environment })
      });
    }
    ...
    const preview = await withConnection(connection, async (pool) => previewWrite(pool, query, {
      runHandle,
      sampleLimit: WRITE_PREVIEW_LIMIT,
      outputPreview: Boolean(getSourceDefinition(connection.sourceType).supportsOutputPreview)
    }));
    ...
    return res.json({
      ...
      environment: target.environment,
      profileName: target.profileName,
      // Returned to the browser only; deliberately not part of the confirmation record, which
      // is persisted to disk, or the audit entry, which must never hold row values.
      previewSample: preview.sample || null,
      previewSampleNote: preview.sampleUnavailable || '',
      message: buildWriteReviewMessage(
        { ...classification, requiresAcknowledgement: Boolean(previewExpectedText) },
        preview.rowsAffected,
        { expectedText: previewExpectedText, environment: target.environment }
      )
    });
  } catch (error) {
...
export async function postSavedConnections(req, res) {
  ...
  addAuditEntry({
    ...
    detail: `Saved connection profile ${saved.profileName || saved.database}${saved.environment ? ` (${saved.environment})` : ''}.`
  });
...
export async function getSavedQueries(_req, res) {
  const items = await listSavedQueries();
  return res.json({ success: true, items });
}

export async function postSavedQueries(req, res) {
  const saved = await upsertSavedQuery(req.body || {});
  // The audit names the saved query, never its SQL: the library can hold literal values.
  addAuditEntry({
    event: 'saved_query_upsert',
    outcome: 'success',
    action: 'SAVE_QUERY',
    detail: `Saved query ${saved.folder ? `${saved.folder} / ` : ''}${saved.name}.`
  });
  return res.json({ success: true, item: saved });
}

export async function deleteSavedQueries(req, res) {
  const id = String(req.body?.id || req.query?.id || '').trim();
  const removed = await deleteSavedQuery(id);
  if (!removed) {
  ...
  addAuditEntry({
    event: 'saved_query_delete',
    outcome: 'success',
    action: 'DELETE_QUERY',
    detail: `Deleted saved query ${id}.`
  });
  return res.json({ success: true, id });
}
