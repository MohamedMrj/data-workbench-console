...
import { NO_RUN, cancelRun, isCancelError, normalizeRunId, registerRun } from './run-registry.js';
import {
...
export async function postQuery(req, res) {
  ...
  try {
    if (classification.kind === 'read') {
      ...
      return res.json({ success: true, mode: 'read', ...data, elapsedMs: Date.now() - startedAt });
    }
