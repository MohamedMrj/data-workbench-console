...
import { addAuditEntry, getAuditConfig, getAuditEntries, loadAuditEntriesFromDisk } from './audit-store';
import { createConfirmation, deleteConfirmation, getConfirmation, hashConfirmationParts, loadConfirmationStore, purgeExpiredConfirmations } from './confirmation-store';
import { deleteSavedConnection, initializeSavedConnectionsStore, listSavedConnections, upsertSavedConnection } from './saved-connections-store';
import { isLoopbackRequestUrl } from './next-handler';
import { getMetadataStoreMode, isAuthRequired, parseAllowedUserEmails } from './live-config';
import {
  classifyQuery,
...
// Startup environment validation â€” warn loudly but do not crash, because
...
function limitReadQuery(query) {
  ...
  return `SELECT TOP (${RESPONSE_ROW_LIMIT}) * FROM (\n${trimmed}\n) AS __rowlimit_wrapper;`;
}
...
function addRequestAuditEntry(req, entry) {
  addAuditEntry(entry, req.userEmail || 'local');
}
...
export function getHealth(_req, res) {
  res.json({
    ...
    live: {
      ...
      allowedUserCount: parseAllowedUserEmails().length,
      metadataStore: getMetadataStoreMode()
    },
...
export function getAudit(req, res) {
  ...
  return getAuditEntries(req.userEmail || 'local', req.query.limit || 25).then((entries) => res.json(entries));
}
...
export async function postObjectInsight(req, res) {
  ...
  if (!['profile', 'dependencies'].includes(action)) {
  ...
  try {
    ...
    const rows = await withConnection(connection, async (pool) => loadObjectDependencies(pool, objectName));
    addRequestAuditEntry(req, {
    ...
    addRequestAuditEntry(req, {
      event: action === 'profile' ? 'object_profile' : 'object_dependencies',
      outcome: 'error',
      action: 'READ_METADATA',
      sourceType: connection.sourceType,
      server: connection.server,
...
export async function getSavedConnections(req, res) {
  const items = await listSavedConnections(req.userEmail || 'local');
  return res.json({ success: true, items });
}

export async function postSavedConnections(req, res) {
  const saved = await upsertSavedConnection(req.userEmail || 'local', req.body || {});
  addRequestAuditEntry(req, {
...
export async function deleteSavedConnections(req, res) {
  ...
  const removed = await deleteSavedConnection(req.userEmail || 'local', id);
  if (!removed) {
