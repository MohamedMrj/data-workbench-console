...
export async function postObjectInsight(req, res) {
  ...
  try {
    if (action === 'resultshape') {
      ...
      const rows = await withConnection(connection, async (pool) => describeQueryResultShape(pool, query));
      addAuditEntry({
      ...
      return res.json({
        ...
        output: { query_columns: rows.length },
        message: `Loaded result shape metadata for ${rows.length} column(s).`
      });
    }
    ...
    if (action === 'rowcount') {
      const data = await withConnection(connection, async (pool) => loadObjectRowCount(pool, objectName, {
        allowCountFallback: Boolean(req.body?.allowCountFallback || req.query?.allowCountFallback === 'true')
      }));
      const rows = [{
  ...
  try {
    const data = await withConnection(leftConnection, async (leftPool) => (
      withConnection(rightConnection, async (rightPool) => (
        loadSchemaCompare(leftPool, rightPool, { leftObject, rightObject, objectType })
      ))
    ));
    addAuditEntry({
...
export async function postQueryPlan(req, res) {
  ...
  try {
    ...
    return res.status(Number(error.httpStatus || 500)).json({
      success: false,
      error: `Estimated plan unavailable: ${error.message}`,
