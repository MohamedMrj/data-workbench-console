...
function acknowledgementForClassification(classification = {}) {
...
function buildWriteReviewMessage(classification = {}, rowsAffected = null) {
  ...
  if (classification.requiresAcknowledgement) {
    return `${action} requires explicit acknowledgement.${warningSuffix} Review the SQL, type EXECUTE ${action}, then click Continue to execute.`;
  }
...
export async function postObjectInsight(req, res) {
  ...
  try {
    if (action === 'editability') {
      ...
      if (!data.keyInfo.keyColumns.length) {
        return res.json({
          success: true,
          action: 'editability',
          editable: false,
          reason: data.keyInfo.unsupported
            ? 'This source does not expose primary key metadata, so a specific row cannot be targeted safely.'
            : 'The table has no primary key or unique constraint, so a specific row cannot be targeted safely.'
        });
      }
      ...
      const keyColumnNamesLower = new Set(data.keyInfo.keyColumns.map((name) => name.toLowerCase()));
      ...
      const editableColumns = data.columns
        ...
        .filter((column) => !keyColumnNamesLower.has(column.name.toLowerCase()))
...
export async function postQuery(req, res) {
  ...
  try {
    ...
    const expectedText = acknowledgementForClassification(classification);
    ...
    return res.json({
      ...
      message: buildWriteReviewMessage(
        { ...classification, requiresAcknowledgement: Boolean(expectedText) },
