...
const ENV_BACKUP_DIR = path.join(process.cwd(), '.data', 'backups');
...
export const ENV_SETTING_GROUPS = [
  ...
  },
  {
    id: 'appearance',
    title: 'Appearance',
    description: 'Controls subtle visual motion and comfort settings for the local UI.'
  },
  {
    id: 'security',
...
export const FIELD_DEFINITIONS = [
  ...
  },
  {
    ...
    description: 'Number of sample rows (before and after) shown in a write preview before confirmation. SQL Server only; Fabric previews show the row count.',
  ...
  },
  {
    ...
    description: 'Write previews above this affected-row count require typing the acknowledgement phrase (for example EXECUTE UPDATE).',
  ...
  },
  {
    ...
    description: 'Maximum rows written by Export all (full CSV/JSON export of a read query). The file notes when it stopped at this limit.',
    appropriate: '100000 suits most exports. Larger values take longer and hold a database connection for the whole download.',
    restartRequired: true
  },
  {
    key: 'EXPORT_REQUEST_TIMEOUT_MS',
    group: 'safety',
    type: 'number',
    min: 10000,
    max: 3600000,
    ...
    description: 'How long a full export may run, including time the download spends waiting for the browser.',
    appropriate: '600000 means 10 minutes.',
    restartRequired: true
  },
  {
  ...
  },
  {
    ...
    description: 'Controls the very slow movement of background colors in the app shell.',
    appropriate: 'true makes the app feel more alive. Use false for a completely static background.',
    restartRequired: true
  },
  {
    ...
    description: 'How long one slow background color movement cycle takes.',
    appropriate: '90000 means 90 seconds. Higher values move more slowly; lower values move more noticeably.',
    restartRequired: true
  },
  {
    key: 'APP_AMBIENT_COLOR',
    group: 'appearance',
    type: 'color',
    defaultValue: '',
    label: 'Ambient color',
    description: 'The color of the moving ambient background wave. Pick any color; the picker previews it live.',
    appropriate: 'Leave blank to follow the current theme accent color, or choose a custom color such as #3fa9ff.',
    restartRequired: true
  },
  {
    key: 'APP_AMBIENT_INTENSITY',
    group: 'appearance',
    type: 'number',
    min: 0,
    max: 100,
    ...
    description: 'How strongly the moving ambient background color shows through the app.',
    appropriate: '0 hides it completely, 55 is a clearly visible default, 100 is the boldest.',
    restartRequired: true
  },
  {
    ...
    description: 'Shows short, delayed hints when hovering or focusing buttons, fields, tabs, and dense controls.',
    appropriate: 'Keep true for newer users. Use false if experienced users prefer a quieter interface.',
    restartRequired: true
  },
  {
    key: 'APP_EDITOR_AUTOCOMPLETE_ENABLED',
    group: 'appearance',
    type: 'boolean',
    defaultValue: 'true',
    label: 'SQL editor autocomplete',
    description: 'Suggests table, view and column names from the loaded catalog while typing in the SQL editor. Ctrl+Space opens suggestions on demand.',
    appropriate: 'Keep true. Use false if the popup gets in the way of fast typing.',
    restartRequired: true
  },
  {
    ...
    defaultValue: '650',
    label: 'Tooltip delay',
    description: 'How long the pointer or keyboard focus must rest on a control before a tooltip appears.',
    appropriate: '650 is calm and avoids flicker. Use 0 for instant hints or 1000-1500 for quieter hints.',
    restartRequired: true
  },
  {
...
function envPaths(options = {}) {
  return {
    envFile: options.envFile || ENV_FILE,
    exampleFile: options.exampleFile || ENV_EXAMPLE_FILE,
    backupDir: options.backupDir || ENV_BACKUP_DIR
  };
}
...
function sanitizeFieldValue(field, rawValue) {
  ...
  if (field.type === 'color') {
    // Empty is allowed and means "follow the theme accent". Otherwise require a
    // #rgb or #rrggbb hex color, normalized to lowercase.
    if (value !== '' && !/^#(?:[0-9a-f]{3}|[0-9a-f]{6})$/i.test(value)) {
      throw new Error(`${field.key} must be a hex color like #3fa9ff, or blank to follow the theme.`);
    }
    value = value.toLowerCase();
  }
...
function fieldForClient(field, values, missingKeys = new Set()) {
  const hasKey = Object.hasOwn(values, field.key);
  const configured = hasKey && values[field.key] !== '';
  return {
    ...field,
...
function missingEnvFields(values = {}) {
  return FIELD_DEFINITIONS.filter((field) => !Object.hasOwn(values, field.key));
}
...
async function backupEnvFile(envFile, backupDir) {
  let envText = '';
  try {
  ...
  const backupPath = path.join(backupDir, `.env-${formatBackupStamp()}.bak`);
  await fs.writeFile(backupPath, envText, 'utf8');
  return backupPath;
}

function appendMissingDefaults(lines, missingFields) {
  ...
  nextLines.push('# Added by Data Workbench settings sync. Review values, then restart if needed.');
  for (const field of missingFields) {
    nextLines.push(`${field.key}=${quoteEnvValue(field.defaultValue ?? '')}`);
  }
  return nextLines;
}

export async function getEnvSettings(options = {}) {
  const paths = envPaths(options);
  const [envText, exampleText] = await Promise.all([
    readOptionalFile(paths.envFile),
    readOptionalFile(paths.exampleFile)
  ]);
  const actualEnv = parseEnvText(envText);
  const env = parseEnvText(envText || exampleText);
  const missingFields = missingEnvFields(actualEnv.values);
  ...
  return {
    ...
    settings: FIELD_DEFINITIONS.map((field) => fieldForClient(field, env.values, missingKeys)),
    envSync: {
      missingKeys: missingFields.map((field) => field.key),
      missingSettings: missingFields.map((field) => ({
        key: field.key,
        label: field.label,
        group: field.group,
      ...
      canSync: missingFields.length > 0
    },
    restartRequired: true
  };
}

export async function syncMissingEnvSettings(options = {}) {
  const paths = envPaths(options);
  const envText = await readOptionalFile(paths.envFile);
  const parsed = parseEnvText(envText);
  const missingFields = missingEnvFields(parsed.values);
  if (!missingFields.length) {
    const settingsPayload = await getEnvSettings(options);
    return {
      ...settingsPayload,
      envPath: paths.envFile,
      changedKeys: [],
  ...
  const backupPath = await backupEnvFile(paths.envFile, paths.backupDir);
  const lines = parsed.entries.map((entry) => entry.raw);
  const nextLines = appendMissingDefaults(lines, missingFields);
  await fs.writeFile(paths.envFile, `${nextLines.join('\n')}\n`, 'utf8');
  ...
  return {
    ...
    envPath: paths.envFile,
    changedKeys: [],
    addedKeys: missingFields.map((field) => field.key),
    backupPath,
    restartRequired: missingFields.some((field) => field.restartRequired)
  };
}

export async function updateEnvSettings(input = {}, options = {}) {
  ...
  const [envText, exampleText] = await Promise.all([
    readOptionalFile(envPaths(options).envFile),
    readOptionalFile(envPaths(options).exampleFile)
  ]);
  ...
  await fs.writeFile(envPaths(options).envFile, `${lines.join('\n')}\n`, 'utf8');

  return {
    envPath: envPaths(options).envFile,
    changedKeys,
    restartRequired: changedKeys.length > 0,
    settings: (await getEnvSettings(options)).settings,
    envSync: (await getEnvSettings(options)).envSync
  };
}
