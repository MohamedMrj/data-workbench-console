...
export function mapRecordset(result, responseRowLimit) {
  if (Array.isArray(result?.recordsets) && result.recordsets.length > 1) {
    const recordsets = result.recordsets.filter((recordset) => Array.isArray(recordset));
    const columnSet = new Set(['result_set']);
    recordsets.forEach((recordset) => {
      const metadataColumns = recordset?.columns && typeof recordset.columns === 'object'
        ? Object.keys(recordset.columns)
        : [];
      const rowColumns = recordset[0] ? Object.keys(recordset[0]) : [];
      [...metadataColumns, ...rowColumns].forEach((column) => columnSet.add(column));
    });
    const columns = [...columnSet];
    const rows = [];
    recordsets.forEach((recordset, recordsetIndex) => {
      recordset.forEach((row) => {
        if (rows.length < responseRowLimit) {
          rows.push({ result_set: recordsetIndex + 1, ...row });
        }
      });
    });
    const totalRows = recordsets.reduce((sum, recordset) => sum + recordset.length, 0);
    return {
      columns,
      rows,
      totalRows,
      truncated: totalRows > responseRowLimit,
...
export function parseQualifiedObjectName(name) {
  // Split on '.' only outside of [bracketed] identifiers, and unquote each part
  // (a closing ']' is escaped as ']]'). This keeps identifiers that legitimately
  // contain '.' or ']' (e.g. [My.Table]) intact instead of mangling them.
  ...
  const objectName = cleanParts.at(-1) || '';
  const schemaName = cleanParts.length >= 2 ? cleanParts.at(-2) : 'dbo';
  return { schemaName, objectName, fullName: composeQualifiedObjectName(schemaName, objectName) };
}

// Compose a fullName that survives being parsed again. The client stores fullName and sends
// it back on every follow-up request, so a plain `schema.object` join loses information for
// an identifier that legitimately contains '.' or ']': `[My.Table]` became `dbo.My.Table`,
// which re-parsed as schema `My`, object `Table`, pointing later requests at the wrong
// object. Bracket-quote a part only when it holds a delimiter, so ordinary names stay
// readable in the explorer, audit details and tab titles.
export function composeQualifiedObjectName(schemaName, objectName) {
  const cleanObject = String(objectName || '');
  if (!cleanObject) {
    return '';
  }
  ...
  return `${encodePart(String(schemaName || 'dbo'))}.${encodePart(cleanObject)}`;
}
...
export async function loadObjects(pool) {
  ...
  return result.recordset.map((row) => ({
    ...
    fullName: composeQualifiedObjectName(row.TABLE_SCHEMA, row.TABLE_NAME),
...
function renderColumnDataType(column) {
  ...
  const usesWideChars = ['nchar', 'nvarchar', 'ntext'].includes(dataType);
...
// Emit an identity seed/increment without losing precision. BIGINT identities can
// exceed 2^53, so coercing through Number() would corrupt the generated script;
// pass a clean integer value through verbatim and fall back only for odd input.
function renderIdentityArgument(value, fallback) {
...
function renderColumnDefinition(column) {
  ...
  const identity = column.is_identity
    ? ` IDENTITY(${renderIdentityArgument(column.seed_value, 1)}, ${renderIdentityArgument(column.increment_value, 1)})`
    : '';
...
// Column-only CREATE TABLE built from INFORMATION_SCHEMA, used when the richer
// catalog DMVs (partition stats, indexes, foreign keys) are not available on the
// source (e.g. a Fabric Lakehouse SQL endpoint). Better than surfacing a raw DMV
// "not supported" error to the user.
async function loadColumnOnlyTableDefinition(pool, parsed, requestedMode) {
  const columns = await loadColumnsForObject(pool, parsed.fullName);
  if (!columns.length) {
    throw createNotFoundError('table', parsed.fullName);
  }

  const tableName = quoteQualifiedObjectName(parsed.fullName);
  const lines = columns.map((column) => (
    `    ${quoteIdentifier(column.name)} ${String(column.type || '').toUpperCase()} ${column.nullable ? 'NULL' : 'NOT NULL'}`
  ));
  const createScript = `CREATE TABLE ${tableName} (\n${lines.join(',\n')}\n);`;
  const definition = requestedMode === 'alter'
    ? `-- Column-only script. Edit into explicit ALTER TABLE statements before running.\n${createScript}`
    : createScript;
  ...
  return {
    ...
    generated: true,
    editable: true,
    definitionSource: 'information_schema_columns_only',
    completeness: {
    ...
    warnings: ['This source exposes column metadata only, so keys, indexes, defaults, identity, and constraints are not included in the generated script.']
  };
}
...
async function loadTableDefinition(pool, parsed, requestedMode) {
  let snapshot;
  try {
    snapshot = await loadTableMetadataSnapshot(pool, parsed.fullName);
  } catch (error) {
    if (isUnsupportedTableMetadataError(error)) {
      return loadColumnOnlyTableDefinition(pool, parsed, requestedMode);
    }
    throw error;
  }
...
export async function loadProcedureCatalog(pool) {
  try {
    ...
    return informationSchemaResult.recordset.map((row) => ({
      ...
      fullName: composeQualifiedObjectName(row.ROUTINE_SCHEMA, row.ROUTINE_NAME)
    }));
  } catch (error) {
...
export async function loadProcedureParameters(pool, procedureName) {
  ...
  return fallbackResult.recordset.map((row) => {
    ...
    const usesWideChars = ['nchar', 'nvarchar', 'ntext'].includes(lowerType);
...
// ─── Row-editability metadata (results-grid inline editor) ──────────────────

// Column data types the inline editor never renders as an input, even when the
// column isn't part of the key. A plain text box can't safely represent these
// without real risk of corrupting the value (binary/spatial/large-object
// types), or is meaningless to hand-edit (timestamp/rowversion is
// auto-generated by SQL Server on every write).
const NON_INLINE_EDITABLE_DATA_TYPES = new Set([
  'binary', 'varbinary', 'image', 'xml',
  'geography', 'geometry', 'hierarchyid',
  'sql_variant', 'timestamp', 'rowversion'
]);

export function isInlineEditableColumnType(dataType) {
  return !NON_INLINE_EDITABLE_DATA_TYPES.has(String(dataType || '').toLowerCase());
}

// The primary key columns for a table, in key-ordinal order, or — when the
// table has no primary key — the columns of the first unique constraint found
// (rows are ordered PK-first then by unique_index_id, so grouping by the
// first unique_index_id present cannot mix columns from two different unique
// constraints together). Returns keyColumns: [] when neither exists, which
// means "no row-editing target can be identified for this table".
export async function loadObjectKeyColumns(pool, objectName) {
  const parsed = parseQualifiedObjectName(objectName);
  if (!parsed.objectName) {
    throw new Error('Object name is required.');
  }
  ...
  try {
    const result = await pool.request()
      ...
      .input('objectName', sql.NVarChar, parsed.objectName)
      .query(`
        SELECT kc.type AS constraint_type, kc.unique_index_id, col.name AS column_name
        FROM sys.key_constraints kc
        INNER JOIN sys.tables tbl ON kc.parent_object_id = tbl.object_id
        INNER JOIN sys.schemas sch ON tbl.schema_id = sch.schema_id
        INNER JOIN sys.index_columns ic ON kc.parent_object_id = ic.object_id AND kc.unique_index_id = ic.index_id
        INNER JOIN sys.columns col ON ic.object_id = col.object_id AND ic.column_id = col.column_id
        WHERE sch.name = @schemaName AND tbl.name = @objectName AND ic.is_included_column = 0
        ORDER BY CASE WHEN kc.type = 'PK' THEN 0 ELSE 1 END, kc.unique_index_id, ic.key_ordinal;
      `);

    const rows = result.recordset || [];
    const primaryKeyRows = rows.filter((row) => row.constraint_type === 'PK');
    if (primaryKeyRows.length) {
      return { keyColumns: primaryKeyRows.map((row) => row.column_name), keyType: 'primary' };
    }

    if (rows.length) {
      const firstUniqueIndexId = rows[0].unique_index_id;
      const uniqueColumns = rows
        .filter((row) => row.unique_index_id === firstUniqueIndexId)
        .map((row) => row.column_name);
      return { keyColumns: uniqueColumns, keyType: 'unique' };
    }

    return { keyColumns: [], keyType: null };
  } catch (error) {
    if (isUnsupportedTableMetadataError(error)) {
      return { keyColumns: [], keyType: null, unsupported: true };
    }
    throw error;
  }
}

// Whether an object is a base table, a view, or does not exist — used to
// restrict inline editing to plain tables. View updatability in SQL Server
// depends on view complexity (single base table, no aggregates/DISTINCT,
// INSTEAD OF triggers) in ways this app does not attempt to determine
// statically, so views are excluded rather than guessed at.
export async function loadObjectKind(pool, objectName) {
  const parsed = parseQualifiedObjectName(objectName);
  if (!parsed.objectName) {
    throw new Error('Object name is required.');
  }
  ...
  const tableType = result.recordset?.[0]?.TABLE_TYPE;
  return {
    exists: Boolean(tableType),
    objectType: tableType === 'BASE TABLE' ? 'table' : tableType === 'VIEW' ? 'view' : null
  };
}
