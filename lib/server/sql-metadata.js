...
export async function loadEstimatedQueryPlan(pool, query) {
  const result = await pool.request().batch(`SET SHOWPLAN_XML ON;\n${String(query || '').replace(/;+\s*$/, '')};\nSET SHOWPLAN_XML OFF;`);
...
export async function loadSchemaCompare(poolLeft, poolRight, { leftObject, rightObject, objectType = 'table' }) {
...
function compareColumnSets(leftObject, rightObject, leftColumns, rightColumns, extra = {}) {
  ...
  return {
    ...
    warnings: ['Schema compare v1 focuses on columns and table metadata available through catalog views; source-specific physical options may not be represented.']
  };
}
