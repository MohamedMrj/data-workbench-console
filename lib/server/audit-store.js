...
export function addAuditEntry({
  ...
  const entry = {
    ...
    ownerEmail: String(ownerEmail || 'local').trim().toLowerCase() || 'local',
    event,
    outcome,
    action,
...
function normalizeAuditRow(row = {}) {
  return {
    ...
    rowCount: row.rowCount ?? row.row_count ?? null,
    detail: String(row.detail || '')
  };
}

export async function getAuditEntries(ownerEmail = 'local', limit = 25) {
  ...
  if (hasSupabaseConfig()) {
    ...
    const { data, error } = await client
      .from('audit_entries')
      .select('timestamp, owner_email, event, outcome, action, source_type, server, database, row_count, detail')
      .eq('owner_email', String(ownerEmail || 'local').trim().toLowerCase() || 'local')
      .order('timestamp', { ascending: false })
      .limit(safeLimit);

    if (error) {
      throw new Error(`Could not load audit entries: ${error.message}`);
    }

    return {
      success: true,
      entries: (data || []).map((row) => normalizeAuditRow(row)),
      limit: safeLimit
    };
  }
  ...
  return {
    ...
    entries: auditEntries
      .filter((entry) => !entry.ownerEmail || entry.ownerEmail === String(ownerEmail || 'local').trim().toLowerCase())
      .slice(0, safeLimit),
    limit: safeLimit
  };
}
