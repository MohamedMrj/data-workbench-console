...
import { isKnownSourceType, normalizeServer, normalizeSourceType } from './source-config.js';
...
// Keep the newest entries that fit inside AUDIT_LOG_MAX_BYTES and drop the oldest overflow.
// Entries are newest-first in memory, so accumulate from the front and reverse at the end so
// the file stays oldest-first. Always keep at least one entry, so an entry larger than the
// whole budget cannot empty the file.
function auditLinesWithinByteCap(entries) {
  const kept = [];
  let bytes = 0;

  for (const entry of entries.slice(0, AUDIT_LOG_LIMIT)) {
    const line = JSON.stringify(entry);
    const lineBytes = Buffer.byteLength(`${line}\n`, 'utf8');
    if (kept.length && bytes + lineBytes > AUDIT_LOG_MAX_BYTES) {
      break;
    }
    kept.push(line);
    bytes += lineBytes;
...
async function rewriteAuditFile(entries = auditEntries) {
  ...
  const lines = auditLinesWithinByteCap(entries).join('\n');

  await fs.writeFile(tmpPath, lines ? `${lines}\n` : '', 'utf8');
  await safeReplaceFile(tmpPath, AUDIT_LOG_FILE);
}

// Persist by always rewriting the file from the in-memory entries (the source of
// truth, capped at AUDIT_LOG_LIMIT). Mixing per-entry appends with size-triggered
// full rewrites could duplicate or drop the newest entries when a rewrite ran
// between an entry's append being queued and executed; a single serialized
// rewrite-from-memory is consistent and idempotent.
function queueAuditWrite() {
  auditWriteQueue = auditWriteQueue
    .then(() => rewriteAuditFile())
    .catch(async (error) => {
...
export function getAuditEntries(limit = 25, filters = {}) {
  ...
  const rawSourceTypeFilter = String(filters.sourceType || '').trim();

  // An unrecognised source-type filter must match nothing. Normalizing it first would turn
  // a typo into the default source and quietly return that source's rows instead.
  if (rawSourceTypeFilter && !isKnownSourceType(rawSourceTypeFilter)) {
    return { success: true, entries: [], limit: safeLimit, totalMatched: 0 };
  }
  ...
  const hasSourceTypeFilter = rawSourceTypeFilter;
