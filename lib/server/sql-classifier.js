 ...
 * Returns `{ kind: 'blocked', reason }` if multiple statements are detected.
 *
 * @param {string} query
...
export function classifyLeadingStatement(query) {
  ...
  if (statements.length > 1) {
    ...
    if (tokens[0] === 'IF' && tokens.includes('BEGIN') && tokens.includes('END')) {
      return {
        kind: 'blocked',
        reason: 'T-SQL IF/BEGIN/END batches are blocked in SQL Studio because they contain multiple executable statements. Put this logic in a stored procedure, or run one reviewed statement at a time.'
      };
    }
    return { kind: 'blocked', reason: 'Only one SQL statement at a time is allowed.' };
  }
 ...
 *   { kind: 'write', action, sanitized, warnings, directConfirmOnly }
 *
 * @param {string} query
 * @returns {object}
 */
export function classifyQuery(query) {
  ...
  if (keyword === 'UPDATE' && !leading.tokens.includes('WHERE')) {
    writeWarnings.push('UPDATE does not include a WHERE clause.');
  }

  if (keyword === 'DELETE' && !leading.tokens.includes('WHERE')) {
    writeWarnings.push('DELETE does not include a WHERE clause.');
  }
  ...
  if ([
    ...
    return {
      ...
      directConfirmOnly: ['MERGE', 'TRUNCATE', 'DROP', 'ALTER', 'CREATE', 'GRANT', 'REVOKE', 'EXEC', 'EXECUTE'].includes(keyword)
    };
  }

  if (keyword === 'SELECT' || keyword === 'WITH') {
    return { kind: 'read', action: 'SELECT', sanitized };
  }
