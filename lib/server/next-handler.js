...
export async function runHandler(handler, req, { params } = {}) {
  ...
  if (!user.authenticated) {
    return finalizeResponse({ success: false, error: 'Authentication required.' }, 401, context, {
      'Cache-Control': 'no-store'
    });
  }

  if (!user.allowed) {
    return finalizeResponse({ success: false, error: 'This account is not allowed to access Data Workbench Console.' }, 403, context, {
      'Cache-Control': 'no-store'
    });
  }
