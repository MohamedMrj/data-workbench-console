...
function validateSameOrigin(req, url) {
  ...
  const compareOrigin = (candidate) => {
    ...
    // Browsers treat localhost, 127.0.0.1, and [::1] as distinct origins, and
    // Next can report req.url's host differently from how the browser connected
    // (e.g. server bound to 127.0.0.1 but req.url host is localhost). For a
    // loopback-only desktop app these are the same machine, so accept any
    // loopback host on both sides as long as the scheme and port still match.
...
export async function runHandler(handler, req, { params } = {}) {
  ...
  const res = {
    ...
    // A handler that returns a file (a full export) hands over a ReadableStream instead of
    // JSON. It still went through the same-origin, rate-limit and session steps above.
  ...
  try {
    ...
    if (streamBody) {
      return finalizeResponse(null, statusCode, context, extraHeaders, streamBody);
    }
...
function finalizeResponse(payload, statusCode, context, extraHeaders = {}, streamBody = null) {
  const init = {
    status: statusCode,
    headers: {
      'Cache-Control': 'no-store',
  ...
  const response = streamBody ? new NextResponse(streamBody, init) : NextResponse.json(payload ?? {}, init);
