...
export const SOURCE_DEFINITIONS = {
  'fabric-sql': {
    ...
    // Fabric Warehouse rejects OUTPUT clauses, so its write previews report a row count only.
    supportsOutputPreview: false
  },
  'fabric-lakehouse': {
    id: 'fabric-lakehouse',
  ...
  'sql-server': {
    id: 'sql-server',
    label: 'SQL Server',
    authModes: ['sqlLogin', 'windowsNtlm', 'servicePrincipal'],
    supportsProcedures: true,
    supportsObjects: true,
    supportsOutputPreview: true
  }
};
...
// normalizeSourceType() intentionally coerces anything unrecognised to the default source,
// which is wrong for callers that filter by source type: an unknown filter value would
// silently become a valid one. Those callers check this first.
export function isKnownSourceType(sourceType) {
  const raw = String(sourceType || '').trim();
  if (!raw) {
    return false;
  }
  return Boolean(SOURCE_DEFINITIONS[raw] || SOURCE_TYPE_ALIASES[raw.toLowerCase()]);
}
...
export function buildConfig(connectionInput) {
  ...
  if (connection.authMode === 'windowsNtlm') {
    if (!connection.domain || !connection.username || !connection.password) {
      throw new Error('Domain, username, and password are required for Windows authentication.');
    }
    config.authentication = {
      type: 'ntlm',
      options: {
